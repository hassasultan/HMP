...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('from_date') && $request->from_date != '' && $request->has('to_date') && $request->to_date != '') {
            $order = $order->whereBetween('created_at', [$request->from_date.' 00:00:00', $request->to_date.' 23:59:59']);
        }
    ...
    public function truck_selection_list(Request $request)
    {
        if (auth()->user()->role != 1) {
            ...
            })->where('black_list',1)->where('status',1)->take(8)->get();
        } else {
            $truck = Truck::with('hydrant', 'truckCap', 'drivers')->where('black_list',1)->where('status',1)->where('name', 'like', '%' . $request->name . '%')->orwhere('company_name', 'like', '%' . $request->name . '%')->orwhere('truck_num', 'like', '%' . $request->name . '%')->take(8)->get();
        }
        return $truck;
