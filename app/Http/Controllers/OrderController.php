...
class OrderController extends Controller
{
    ...
    public function truck_selection_list(Request $request)
    {
        if (auth()->user()->role != 1) {
            ...
            $truck = Truck::with('hydrant', 'truckCap', 'drivers')->where('status',1)->where('name', 'like', '%' . $request->name . '%')->orwhere('company_name', 'like', '%' . $request->name . '%')->orwhere('truck_num', 'like', '%' . $request->name . '%')->take(8)->get();
        }
        return $truck;
