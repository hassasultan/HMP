...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(auth()->user()->role == 1)
        {
            ...
            $today_dc = Orders::with('customer')->where('order_type',"Dc quota")->whereDay('created_at', '=', date('d'))->count();
        ...
        else
        {
            ...
            $today_dc = Orders::with('customer')->where('order_type',"Dc quota")->where('hydrant_id',auth()->user()->hydrant_id)->whereDay('created_at', '=', date('d'))->count();
